export default function FinDeRondaModal({ finDeRonda, premio, onCerrar }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md px-6 animate-fadeIn">
      <div className="glass-card w-full max-w-sm max-h-[90vh] overflow-y-auto animate-scaleIn border-emerald-500/20 shadow-[0_0_50px_rgba(16,185,129,0.1)]">
        <div className="p-8">
          {/* Header */}
          ...
          {/* Ganador o empate */}
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-6 mb-6 text-center shadow-[0_0_20px_rgba(16,185,129,0.1)]">
            {empate ? (
              <>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-3">
              {ranking.map((item, index) => (
                <div
                  key={item.user.id}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${index === 0 && item.puntos > 0
          ...
          <button
            onClick={onCerrar}
            className="w-full py-4 btn-aesthetic text-lg tracking-wide hover:scale-[1.02] shadow-[0_0_25px_rgba(16,185,129,0.4)]"
          >
            Continuar
          </button>
        </div>
